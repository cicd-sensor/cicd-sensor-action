...
function pipeIntoCtl(subcommand, resultLogPath, outputPath) {
  ...
  try {
    const r = spawnSync(CTL, ['report', subcommand, '--output-path', outputPath], {
...
function runtimeTelemetryDebugPath(outDir) {
  const debugOutputDir = core.getState(STATE.debugOutputDir) || path.join(outDir, 'debug');
  return path.join(debugOutputDir, DEBUG_RUNTIME_TELEMETRY_LOG);
}
...
function teardownDockerProxy() {
  spawnSync('sudo', ['systemctl', 'stop', PROXY_UNIT_NAME], { stdio: 'inherit' });
  if (fs.existsSync(DOCKER_SOCKET)) {
    spawnSync('sudo', ['rm', '-f', DOCKER_SOCKET], { stdio: 'inherit' });
  }
  ...
  try {
    ...
    if (dockerProxyEnabled) {
      try { teardownDockerProxy(); } catch (err) {
        core.warning(`docker proxy teardown failed: ${err && err.message ? err.message : err}`);
      }
    }
  }
}
